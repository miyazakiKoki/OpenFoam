...
int main(int argc, char *argv[])
{
    ...
    List<vector> Force(timeDirs.size(),vector::zero);
    ...
    List<scalar> h(timeDirs.size());
    forAll(h,i)
    {
        h[i]=particleSeparationDistance[i]/2;
    }
    
    List<scalar> noDimensionH(timeDirs.size());
    forAll(noDimensionH,i)
    {
        noDimensionH[i]=h[i]/(particleDiameter/2);
    }
    
    List<scalar> scalarForce(timeDirs.size());
    forAll(scalarForce,i)
    {
        scalarForce[i] = mag(Force[i]);
    }
    
    List<scalar> noDimensionForce(timeDirs.size());
    forAll(noDimensionForce,i)
    {
        noDimensionForce[i]=scalarForce[i]/(2*3.14*surfaceTension*particleDiameter/2);
    }
    
    
    particleSeparationDistance[0]=particleSeparationDistance[1];
    noDimensionForce[0]=noDimensionForce[1];
    noDimensionH[0]=noDimensionH[1];
    ...
    Info<< "Writing to the output file"<<endl;
        ...
        forAll(particleSeparationDistance,i)
        {
            ...
            osB <<particleSeparationDistance[i]<<endl;  
                     
        }
     
        
    fileName rootNameC(runTime.path());
    fileName fNameC("noDimensionParticleForce");
    OFstream osC(rootNameC/fNameC);
    Info<< "Writing to the output file"<<endl;

        forAll(noDimensionForce,i)
        {
            
            osC <<noDimensionForce[i]<<endl;  
                     
        }
        
    fileName rootNameD(runTime.path());
    fileName fNameD("noDimensionH");
    OFstream osD(rootNameD/fNameD);
    Info<< "Writing to the output file"<<endl;

        forAll(noDimensionH,i)
        {
            
            osD <<noDimensionH[i]<<endl;  
